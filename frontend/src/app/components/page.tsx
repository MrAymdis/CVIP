...
import { Search, Filter, Building, Tag, AlertCircle, Bug } from 'lucide-react';
...
export default function ComponentsPage() {
  ...
  useEffect(() => {
    fetchComponents();
  }, [searchKeyword, vendorFilter, page]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {components.map((component) => (
                ...
                >
                  ...
                  {component.related_vuln_ids && component.related_vuln_ids.length > 0 && (
                    <div className="mt-3 pt-3 border-t">
                      <div className="flex items-center gap-2 mb-2">
                        <Bug className="h-4 w-4 text-muted-foreground" />
                        <span className="text-xs text-muted-foreground">关联漏洞ID:</span>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {component.related_vuln_ids.slice(0, 5).map((vuln_id, index) => (
                          <Link
                            key={index}
                            href={`/vuln/${vuln_id}`}
                            className="px-2 py-1 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 rounded text-xs hover:bg-red-100 dark:hover:bg-red-900 transition-colors font-mono"
                          >
                            {vuln_id}
                          </Link>
                        ))}
                        {component.related_vuln_ids.length > 5 && (
                          <span className="px-2 py-1 bg-muted text-muted-foreground rounded text-xs">
                            +{component.related_vuln_ids.length - 5} 更多
                          </span>
                        )}
                      </div>
                    </div>
